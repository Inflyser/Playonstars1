...
const initializeApp = async () => {
  try {
    ...
    if (isTelegram) {
      // 3. Загружаем данные пользователя ВНЕ ЗАВИСИМОСТИ от того, есть ли уже user в сторе
      try {
        // Важно: сначала получаем данные пользователя
        await userStore.fetchUserData();
        
        // Затем загружаем баланс
        await userStore.fetchBalance();
        
        // Подключаем WebSocket для реальных обновлений
        await connectWebSocket();
        
        console.log('✅ Данные пользователя и WebSocket успешно инициализированы');
      } catch (err) {
